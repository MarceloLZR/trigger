...
class ProcessWorker(QThread):
    log = Signal(str)
    progress = Signal(int)
    finished_ok = Signal(object, object)     # (DataFrame, ExecutionRecord)
    ...
    def run(self):
        ...
        try:
            ...
            rendered_sql = self.template_engine.render(raw_sql, self.params)
            ...
            # El script puede tener múltiples statements (GO no es válido en pyodbc,
            # se asume que el .sql usa ';' o batches separados por línea "-- BATCH --")
            ...
            self.log.emit(f"Leyendo tabla final: {self.process.final_table}")
            df = pd.read_sql(f"SELECT * FROM {self.process.final_table}", conn)
            ...
            self.log.emit(f"Proceso finalizado: {record.row_count} registros obtenidos.")
            self.finished_ok.emit(df, record)
    ...
    def _split_batches(sql_text: str) -> list[str]:
        """SQL Server usa GO como separador de batch (no válido en pyodbc).
        Dividimos por líneas que sean exactamente 'GO'."""
